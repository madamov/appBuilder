...
# 4D build result destination folder is in $1
destinationFolder=$1
build=$2
version=$3
...
if [ -z "$repoURL" ]; then
	...
	mkdir $thisBuildDestinationFolder/release_repo/
	git clone https://$RUNNER_ACTOR:$WHO_TO_TRUST@$repoURL $thisBuildDestinationFolder/release_repo
	echo "🐚🐚 : Deleting old files from repo..."
	rm -R $thisBuildDestinationFolder/release_repo/*
	rm -R $thisBuildDestinationFolder/release_repo/.gitignore
	# echo "🐚🐚 : Copying files ..."
	...
	gh release upload "$releasetag" "$thisBuildDestinationFolder/${appName}_struct.zip"
